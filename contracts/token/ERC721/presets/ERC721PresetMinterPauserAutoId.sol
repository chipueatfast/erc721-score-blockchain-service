...
contract ERC721PresetMinterPauserAutoId is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
    ...
    mapping (address => string) _candidates;
    ...
    function registerCandidate(address candidateAddress, string memory name) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have admin role to register candidate");
        require(keccak256(bytes(_candidates[candidateAddress])) == keccak256(bytes("")), "This address has already been registered");
        _setupRole(CANDIDATE_ROLE, _msgSender());
        _candidates[_msgSender()] = name;
    }
    function grantAuditorRole(address toGrantAuditorRole) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have admin role to grant");
        _setupRole(MINTER_ROLE, toGrantAuditorRole);
    }
    ...
    function getOwnerAddressByTokenId(uint256 tokenId) public view returns (address)  {
        return _owners[tokenId];
    }

    /**
     * @dev Creates a new token f<p></p>or `to`. Its token ID will be automatically
     ...
     */
    function mint(address to, string memory scorehash) public virtual {
